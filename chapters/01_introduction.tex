...
The latter obtained, in classical mechanics, by scaling the velocity by a constant being in this way equivalent to it.
...
Solving a classical mechanics problem means to solve a set of differential equations.
  ...
  \begin{multicols}{2}
    \begin{enumerate}
      \item A body will continue its state at rest or at constant speed unless a force acts.
        It is only true in inertial frames.
        The lack of an initial consensus inertial frames raises issues.
  ...
  Its corollary is that trajectories in the phase-space can never intersect.
...
Defining a physical theory in classical mechanics corresponds to specifying what is a force.
  ...
  It can be noted that from $H(p,q)$ Newton's equation of motion can be retrieved, solved an prediction to compare with experimental results can be obtained following:
  ...
  This procedure is repeated until concordance with the experiments is reached.
...
The hydrogen atom is formed by a proton in the centre with an electron orbiting around it.
  ...
  \begin{multicols}{2}
    ...
    \begin{itemize}
      ...
      \item  Because $\frac{d{}}{d{t}}\vec{L}=0$ because of angular momentum conservation, the electron's motion occurs on a plane and is two dimensional.
    \end{itemize}
  \end{multicols}

  \subsection{Newton's equation}
  Let us start from the Newton's equations.
  ...
  Since the electron is moving its coordinates $x$, $y$, $z$ are varying with time and they can be expressed as $x(t)$, $y(t)$ and $z(t)$.
